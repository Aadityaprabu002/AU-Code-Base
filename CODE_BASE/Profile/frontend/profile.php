<?php
include('C:\xampp\htdocs\AU_CODING_PLATFORM\CODE_BASE\Profile\backend\profile.php');
?>

<!DOCTYPE html>
<html lang="en">

<head>
  <meta charset="utf-8">
  <title>
    <?php

    ?>
  </title>
  <meta name="viewport" content="width=device-width, initial-scale=1">
  <link rel="stylesheet" href="profile.css">
  <link rel="stylesheet" href=<?php echo (ASSESTS::BADGES_CSS); ?>>
  <link href="http://netdna.bootstrapcdn.com/bootstrap/4.4.1/css/bootstrap.min.css" rel="stylesheet">
</head>

<body>
  <nav class="navbar navbar-expand-lg navbar-dark text-uppercase">
    <a class="navbar-brand" href=<?php echo (LINK::HOMEPAGE); ?>><img src=<?php
                                                                          echo (ASSESTS::LOGO);
                                                                          ?> width="60"></a>
    <button class="navbar-toggler" type="button" data-toggle="collapse" data-target="#navbarNavAltMarkup" aria-controls="navbarNavAltMarkup" aria-expanded="false" aria-label="Toggle navigation">
      <span class="navbar-toggler-icon"></span>
    </button>
    <div class="collapse navbar-collapse" id="navbarNavAltMarkup">
      <div class="navbar-nav">
        <a class="nav-item nav-link" href=<?php echo (LINK::HOMEPAGE); ?>>HOME</a>

        <a class="nav-item nav-link" href=<?php echo (LINK::LEADERBOARD); ?>>LEADERBOARD</a>
        <a class="nav-item nav-link" href=<?php echo (LINK::CHALLENGES); ?>>CHALLENGE</a>
        <a class="nav-item nav-link " target="_blank" href=<?php echo (LINK::BADGECHART); ?>>Badge chart</a>
      </div>
    </div>
    <?php
    if (!$_GET) {
    ?>
      <div class="button-item">
        <form action=<?php echo (LINK::LOGOUT) ?> method="POST">
          <input name="logout" type="submit" class="btn btn-light profile-btn" value="LOGOUT">
        </form>
      </div>
    <?php
    }
    ?>
  </nav>

  <div class=" mid-container">
    <div class="card" style="width: 18rem;">
      <!-- <span class="profile-image">Dp</span> -->

      <div class="card-body">
        <h5 class="card-title text-center"><?php echo ($userDetails['name']); ?></h5>
        <?php
        if ($userDetails["bio"]) {
        ?>
          <p class="card-text"><?php echo ($userDetails['bio']); ?></p>
        <?php } ?>
      </div>
      <ul class="list-group list-group-flush">
        <li class="list-group-item">Badges: <?php echo (count($userDetails['badges'])); ?></li>
        <li class="list-group-item">Points: <?php echo ($userDetails['points']); ?></li>
      </ul>
      <div class="card-body">
        <!-- <i class="fab fa-github-square"></i> -->
        <?php
        if ($userDetails["github"]) {
        ?>
          <a href=<?php echo ($userDetails["github"]) ?> class="card-link github-icon">
            <svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" fill="currentColor" class="bi bi-github" viewBox="0 0 16 16">
              <path d="M8 0C3.58 0 0 3.58 0 8c0 3.54 2.29 6.53 5.47 7.59.4.07.55-.17.55-.38 0-.19-.01-.82-.01-1.49-2.01.37-2.53-.49-2.69-.94-.09-.23-.48-.94-.82-1.13-.28-.15-.68-.52-.01-.53.63-.01 1.08.58 1.23.82.72 1.21 1.87.87 2.33.66.07-.52.28-.87.51-1.07-1.78-.2-3.64-.89-3.64-3.95 0-.87.31-1.59.82-2.15-.08-.2-.36-1.02.08-2.12 0 0 .67-.21 2.2.82.64-.18 1.32-.27 2-.27.68 0 1.36.09 2 .27 1.53-1.04 2.2-.82 2.2-.82.44 1.1.16 1.92.08 2.12.51.56.82 1.27.82 2.15 0 3.07-1.87 3.75-3.65 3.95.29.25.54.73.54 1.48 0 1.07-.01 1.93-.01 2.2 0 .21.15.46.55.38A8.012 8.012 0 0 0 16 8c0-4.42-3.58-8-8-8z" />
            </svg>
          </a>

        <?php } ?>
        <?php
        if ($userDetails["twitter"]) {
        ?>
          <a href=<?php echo ($userDetails["twitter"]) ?> class="card-link">
            <svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" fill="currentColor" class="bi bi-twitter" viewBox="0 0 16 16">
              <path d="M5.026 15c6.038 0 9.341-5.003 9.341-9.334 0-.14 0-.282-.006-.422A6.685 6.685 0 0 0 16 3.542a6.658 6.658 0 0 1-1.889.518 3.301 3.301 0 0 0 1.447-1.817 6.533 6.533 0 0 1-2.087.793A3.286 3.286 0 0 0 7.875 6.03a9.325 9.325 0 0 1-6.767-3.429 3.289 3.289 0 0 0 1.018 4.382A3.323 3.323 0 0 1 .64 6.575v.045a3.288 3.288 0 0 0 2.632 3.218 3.203 3.203 0 0 1-.865.115 3.23 3.23 0 0 1-.614-.057 3.283 3.283 0 0 0 3.067 2.277A6.588 6.588 0 0 1 .78 13.58a6.32 6.32 0 0 1-.78-.045A9.344 9.344 0 0 0 5.026 15z" />
            </svg>
          </a>
        <?php  } ?>
      </div>
    </div>
    <div class="user-dashboard overflow-auto .smooth-scroll">

      <div class="user-rank">
        <center>
          <h1>RANK:<?php echo ($userDetails['rank']); ?></h1>
        </center>
      </div>
      <div class="user-badges" id="user-badges">
        <?php
        foreach ($userDetails["badges"] as $code) {
          echo (getBadge($code));
        }
        ?>
      </div>
    </div>

  </div>
  <br><br><br><br><br>
  <?php include('C:\xampp\htdocs\AU_CODING_PLATFORM\CODE_BASE\footer\frontend\footer.php');
  ?>

  <script src="https://code.jquery.com/jquery-3.4.1.slim.min.js" integrity="sha384-J6qa4849blE2+poT4WnyKhv5vZF5SrPo0iEjwBvKU7imGFAV0wwj1yYfoRSJoZ+n" crossorigin="anonymous"></script>
  <script src="https://cdn.jsdelivr.net/npm/popper.js@1.16.0/dist/umd/popper.min.js" integrity="sha384-Q6E9RHvbIyZFJoft+2mJbHaEWldlvI9IOYy5n3zV9zzTtmI3UksdQRVvoxMfooAo" crossorigin="anonymous"></script>
  <script src="https://stackpath.bootstrapcdn.com/bootstrap/4.4.1/js/bootstrap.min.js" integrity="sha384-wfSDF2E50Y2D1uUdj0O3uMBJnjuUD4Ih7YwaYd1iqfktj0Uod8GCExl3Og8ifwB6" crossorigin="anonymous"></script>
  <script src="https://kit.fontawesome.com/761bece813.js" crossorigin="anonymous"></script>
  <script src="profile.js"></script>
</body>

</html>